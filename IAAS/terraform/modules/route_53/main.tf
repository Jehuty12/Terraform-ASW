...
resource "aws_acm_certificate" "certificate_iaas" {
    domain_name = var.iaas_domain_name
...
resource "aws_route53_record" "paas_certificate_record" {
    for_each = {
      for dvo in aws_acm_certificate.certificate_iaas.domain_validation_options : dvo.domain_name => {
